...
def receberEstado(conexao, protocolo, estado_jogo, lock, cargo):
    while estado_jogo["rodando"]:
        ...
        if dados:
            with lock:
                # O cliente atualiza seu estado com base nos dados recebidos do host
                if cargo == "cliente":
                    if "bolax" in dados:
                ...
                # Ambos atualizam a posição da raquete do oponente
                if "y" in dados:
                    estado_jogo["raqueteOponente_y"] = dados["y"]

                # Verifica se recebeu sinal para encerrar
